 ...
 * @author Yves
 */
public class ImageCollection extends ObjectCollectionObject {
    ...
    public SubObjects getSubObjects() {
        SubObjects<Image> so = new SubObjects();
    ...
    public void sortByFrames(){
        Collections.sort(texts, new Comparator<Image>() {
            @Override
    ...
    public Image getBefore(int frame){
        Image before = null;
        for(Image tx : texts){
    ...
    public Image getAfter(int frame){
        Image after = null;
    ...
    public void setList(List<Image> texts){
        this.texts = texts;
    }
    ...
    public BufferedImage getFX(int frame, int imageWidth, int imageHeight, boolean encoding) {
        ...
        Image before = getBefore(frame);
        Image after = getAfter(frame);
...
//            }            
        ...
        //Couleur
        //Color c = Image.getActualColor(before, after, frame);
        //Taille de la fonte
        //float size = Image.getActualSize(before, after, frame);
        ...
        float x = Image.getActualX(before, after, anchor, frame);
        float y = Image.getActualY(before, after, anchor, frame);
        //Transparence
        float transparency = Image.getActualTransparency(before, after, frame);
        //Echelle X et Y
        float scale_x = Image.getActualScaleX(before, after, frame);
        float scale_y = Image.getActualScaleY(before, after, frame);
        //Angle
        float angle = Image.getActualAngle(before, after, frame);
        //Dégradé (deux cotés)
        //Color[] twosidesgradient = Image.getActualGradientColor(before, after, frame);
        //Dégradé (quatre cotés)
        //Color[] foursidesgradient = Image.getActualFourSidesGradientColor(before, after, frame);
        //Image
        BufferedImage addedImage = Image.getActualImage(before, frame);
        ...
        graFX.setImageOntoFrames(addedImage);
        
        if(scriptObject!=null){
            return graFX.getImageFromScript(scriptObject, frame);
        }
        
        return graFX.getImageFromImage();
    }
